...
//		params.put("cart_key", CommonUtils.generateUUID());
	...
	public String createOrder(Map<String,Object> params) throws Exception{
		...
		if (paymentMethodId == null || paymentMethodId.equals("")) paymentMethodId = "1";
//		Map<String,Object> pay = this.getBaseDao().get(PRIFIX + ".findPaymementMethodNameById", paymentMethodId.toString());
//		if(pay != null) {
...
//		}
		...
		if(shippingMethodId == null || shippingMethodId.equals("") ) shippingMethodId = "1";
//		Map<String,Object> ship = this.getBaseDao().get(PRIFIX + ".findShipmementMethodNameById", shippingMethodId.toString());
//		if(ship != null) {
...
//		}
	...
	private void createOrderItems(Map<String,Object> params,Object id) throws NumberFormatException, Exception {
		...
		Map<String,Object> productMap = this.getBaseDao().get(PRIFIX + ".findProductDetail", Long.parseLong(productId.toString()));
		...
		long product_main_category = productMap.get("product_main_category") == null ? 0 : Long.parseLong(productMap.get("product_main_category").toString()) ;
...
//				Object buy_time = map.get("buy_time");
				...
				if(create_date != null) map.put("create_date", sdf.format(create_date));
//				if(buy_time != null && StringUtils.isNotBlank(buy_time.toString()))map.put("buy_time", sdf.format(buy_time.toString()));
			}
		}
		return page;
	}
...
//	}
	...
	public Map<String,Object> orderDetail(Map<String,Object> params) throws Exception {
		...
		orderMap.put("create_date", orderMap.get("create_date") != null ? sdf.format((Date)orderMap.get("create_date")) : "");
...
//		this.getBaseDao().delete(PRIFIX + ".deleteCart", params);
		...
		for (String cartItemId : cartIdss) {
			...
			Integer count = countMap.get("cartnum") == null ? null : Integer.parseInt(countMap.get("cartnum").toString());
		...
		if(paymentMap == null) {
			...
			if(orderMap != null) {
				...
				if(StringUtils.isNotBlank(_sn)) {
					List<Map<String,Object>> ordetItems = this.getBaseDao().getList(PRIFIX + ".findOrderItemByOrderId", sn);
					...
					payment.setMethod(0);
					payment.setStatus(0);
					payment.setPaymemntMethod(orderMap.get("payment_method_name") == null ? "1" : orderMap.get("payment_method_name").toString());
					payment.setFee(0.0);
					...
					payment.setMember(orderMap.get("member") == null ? 0L : Long.parseLong(orderMap.get("member").toString()));
...
//		String xml = "<xml><appid><![CDATA[wx2421b1c4370ec43b]]></appid><attach><![CDATA[支付测试]]></attach><bank_type><![CDATA[CFT]]></bank_type>"
